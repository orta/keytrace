...
export class Profile {
     ...
     */
    static async fetch(didOrHandle, serviceUrl) {
     ...
     */
    static async _fetchWithAgent(agent, did) {
    ...
    getSummary() {
        return {
            ...
            failed: this._claims.filter((c) => c.status === ClaimStatus.FAILED || c.status === ClaimStatus.ERROR).length,
            pending: this._claims.filter((c) => c.status === ClaimStatus.INIT || c.status === ClaimStatus.MATCHED).length,
        };
    }
    /**
    ...
    getClaimsByStatus() {
        return {
            ...
            failed: this._claims.filter((c) => c.status === ClaimStatus.FAILED || c.status === ClaimStatus.ERROR),
            pending: this._claims.filter((c) => c.status === ClaimStatus.INIT || c.status === ClaimStatus.MATCHED),
        };
    }
    toJSON() {
        return {
