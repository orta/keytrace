...
export function checkExpect(expectStr: string, actual: unknown): { pass: boolean; message: string } {
  ...
  if (colonIdx === -1) {
    return { pass: false, message: `Invalid expect format: "${expectStr}" (expected "type:value")` };
  }
  ...
  switch (type) {
    case "equals":
      return actualStr === expected
        ? { pass: true, message: `Value equals "${expected}"` }
        : { pass: false, message: `Expected "${expected}" but got "${actualStr}"` };

    case "contains":
      return actualStr.includes(expected)
        ? { pass: true, message: `Value contains "${expected}"` }
        : { pass: false, message: `Expected value to contain "${expected}" but got "${actualStr}"` };
