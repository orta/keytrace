    ...
    <template v-else-if="profile">
      ...
      <div v-if="profile.claims && profile.claims.length > 0" class="mt-8 space-y-3">
        <ClaimCard v-for="claim in profile.claims" :key="claim.uri" :claim="mapClaim(claim)">
          <template #actions>
            <VerifyPopover :claim-uri="claim.uri" :did="profile.did" :display-name="mapClaim(claim).displayName" :provider-name="claim.matches?.[0]?.providerName" :rkey="isOwnProfile ? claim.rkey : undefined" @updated="(newStatus) => { if (newStatus !== claim.status) refresh() }" />
            <button
              v-if="isOwnProfile"
