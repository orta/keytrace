<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <template v-else-if="profile">
      ...
      <!-- Claims list -->
      <div
        v-if="profile.claims && profile.claims.length > 0"
      ...
      <div
        v-else
        class="mt-12 text-center py-16 rounded-xl border border-zinc-800/50 bg-kt-surface"
      >
        ...
        <p class="text-sm text-zinc-500 max-w-sm mx-auto">
          This user hasn't linked any external accounts to their ATProto identity yet.
        </p>
      </div>
      ...
      >
        ...
        <div class="mt-3 p-4 rounded-lg bg-kt-inset border border-zinc-800 font-mono text-xs text-zinc-500 space-y-1">
          ...
          <div v-if="profile.summary">
            Claims: {{ profile.summary.total }} total, {{ profile.summary.verified }} linked
          </div>
        </div>
      </details>
    </template>
...
const { data: profile, pending, error } = await useFetch(
  () => `/api/profile/${encodeURIComponent(cleanHandle.value)}`,
)

// Map API claims to the shape ProfileHeader expects
const profileClaims = computed(() =>
  (profile.value?.claims ?? []).map((c: any) => ({ status: c.status })),
)
...
useHead({
  title: computed(() =>
    profile.value
      ? `${profile.value.displayName || profile.value.handle} - Keytrace`
      : "Profile - Keytrace",
  ),
  meta: [
    {
      ...
      content: computed(() =>
        profile.value
          ? `View ${profile.value.displayName || profile.value.handle}'s verified identities on Keytrace`
          : "View verified identities on Keytrace",
      ),
    },
  ],
})
