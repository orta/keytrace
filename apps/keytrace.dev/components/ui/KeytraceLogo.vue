<template>
  <div class="kt-logo-root" @mousedown="onMouseDown" @mouseup="onMouseUp" @mouseleave="onMouseLeave">
    <svg
      :width="width"
      :height="height"
      ...
      :class="['kt-logo', { 'kt-logo--used': state !== 'idle' }]"
    >
      ...
      <path d="M357.077 109.534H320.21L314.942 106.9L309.675 109.534H278.074L275.44 106.9L270.173 109.534H187.328L178 104.267V99H372.879L357.077 109.534Z" fill="black" />
      <path
    ...
    <span :class="['kt-wordmark', `kt-wordmark--${state}`]" aria-hidden="true">
      <span v-for="(letter, i) in letters" :key="i" class="kt-letter" :style="`--i: ${i}`">{{ letter }}</span>
    </span>
  </div>
</template>
...
const letters = Array.from("Keytrace");

type LogoState = "idle" | "down" | "fading" | "used";
const state = ref<LogoState>("idle");
let fadeTimer: ReturnType<typeof setTimeout> | null = null;

function onMouseDown() {
  if (state.value === "used") return;
  state.value = "down";
}
...
.kt-logo-root:active .kt-logo,
.kt-logo-root:active .kt-logo--used {
  transform: rotate(0deg) translateX(8px);
  filter: brightness(1.15) drop-shadow(0 0 6px rgba(124, 58, 237, 0.6));
...
.kt-wordmark--down .kt-letter {
  animation: letter-shimmer 0.55s ease both;
