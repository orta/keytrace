<template>
  <div class="rounded-xl border border-zinc-800 bg-kt-surface overflow-hidden">
    <div
      ...
      :key="i"
      class="flex items-start gap-3 px-4 py-3 border-b border-zinc-800/50 last:border-b-0"
    >
      <!-- Status indicator -->
      <div class="mt-0.5">
        <CheckCircleIcon
          v-if="step.status === 'success'"
          class="w-4 h-4 text-verified"
        />
        <XCircleIcon
          v-else-if="step.status === 'error'"
          class="w-4 h-4 text-failed"
        />
        <div
        ...
        <div
          v-else
          class="w-4 h-4 rounded-full border border-zinc-700"
        />
      </div>
      ...
      <!-- Timing -->
      <span v-if="step.duration" class="text-xs text-zinc-600 font-mono">
        {{ step.duration }}ms
      </span>
    </div>
  </div>
