 ...
 */
export function signClaim(claimData: Record<string, unknown>, privateKey: crypto.KeyObject): string {
 ...
 */
export function verifyClaim(claimData: Record<string, unknown>, jws: string, publicKey: crypto.KeyObject): boolean {
  ...
  return crypto.verify("SHA256", Buffer.from(signingInput), publicKey, derSig);
}
...
function rawToDer(raw: Buffer): Buffer {
  ...
  return Buffer.concat([Buffer.from([0x30, totalLen, 0x02, rLen]), r, Buffer.from([0x02, sLen]), s]);
}
